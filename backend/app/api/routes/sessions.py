...
async def list_sessions(
    ...
    sessions = await session_repo.list_sessions(user_id=current_user.id)
    return ApiResponse(
        success=True,
...
async def get_session_by_number(
    ...
    s = await session_repo.get_session_by_chat_number(chat_number, user_id=current_user.id)
    if not s:
...
async def get_session(
    ...
    s = await session_repo.get_session(session_id, user_id=current_user.id)
    if not s:
...
async def delete_session(
    ...
    await session_repo.delete_session(session_id, user_id=current_user.id)
    return ApiResponse(
        success=True,
