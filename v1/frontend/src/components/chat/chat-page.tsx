...
export function ChatPage({ chatNumberParam }: { chatNumberParam?: string }) {
  ...
  // If unauthenticated user tries to open a specific shareable chat URL
  useEffect(() => {
    if (!user && chatNumberParam) {
      navigate({
        to: "/login",
        search: { redirect: `/chat/${chatNumberParam}` },
      });
    }
  }, [user, chatNumberParam, navigate]);
  ...
  // Load chat by chatNumberParam when accessing /chat/:chatNumber directly (for authenticated user)
  useEffect(() => {
    if (!chatNumberParam || !user) {
      setNotFound(false);
  ...
  }, [chatNumberParam, user, loadChatByNumber, activeSession?.chat_number, activeSession?.isTemporary]);
  ...
  const handleSend = async (text: string) => {
    ...
    if (isDraft) {
      ...
      try {
        const saved = await createPersistentSessionFromDraft(text, selectedDocIds);
        const targetId = saved?.id;

        if (saved?.chat_number) {
          void navigate({
            to: "/chat/$chatNumber",
            params: { chatNumber: saved.chat_number },
            replace: true,
          });
        }
      ...
      <div className="flex h-full flex-col bg-background">
            ...
            {!user ? (
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  asChild
                >
                  <Link to="/signup" search={{ redirect: "/chat" }}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto scroll-fade px-4 py-6 sm:px-6">
          <div className="mx-auto w-full max-w-3xl space-y-6">
            ...
            {/* Messages (rendered only if messages exist, which occurs for authenticated user) */}
